###
  generate_diary_without_adapter_ite.coffee  —  DIARY_ITE pipeline step
  =====================================================
  Control arm of the diary A/B: generates a diary entry
  via the same base model **without** the LoRA adapter,
  so any quality lift in the with-adapter output can be
  attributed to training rather than prompt variation.
###
