###
  cache_embedding.coffee  —  shared helpers
  =====================================================
  Used by `scripts/kag_oracle_ite/oracle_ask_sqlite.coffee` to extract
  embeddings from `mlx_lm cache_prompt` output, and by
  `scripts/eval_ite/voice_similarity_ite.coffee` to compute cosine
  similarity between completion embeddings and a corpus centroid.

  Why it lives under `scripts/_helpers/`:
    - Used by multiple step scripts; not itself a step.
    - The leading underscore keeps it out of the runner's step-
      discovery glob (which looks at `scripts/<recipe>/<step>.coffee`).

  Safetensors layout produced by `mlx_lm cache_prompt` (verified by
  `test.sh`'s cache_prompt_probe — see results/.../safetensors_header.json):
